...
M2[5] = numpy.ndarray.tolist( 100 * numpy.random.random_sample( (1,2) ) )  # = M[N-1]
M2[4] = numpy.ndarray.tolist( 100 * numpy.random.random_sample( (2,4) ) )
M2[3] = numpy.ndarray.tolist( 100 * numpy.random.random_sample( (4,8) ) )
M2[2] = numpy.ndarray.tolist( 100 * numpy.random.random_sample( (8,4) ) )
M2[1] = numpy.ndarray.tolist( 100 * numpy.random.random_sample( (4,2) ) )
M2[0] = numpy.ndarray.tolist( 100 * numpy.random.random_sample( (2,1) )  ) '''
...
for k in range(N-1,-1,-1):  #[N-1, ..., 0] technically dont get the trivial identity Lambda here
    ...
    S[k] = tripledot( Pinv, Uinv[k+1], Mtilde[k] )
    #print "S[",k,"] is ", S[k]
    ...
    print "P[",k,"] is ", P
    print "Pinv[",k,"] is ", Pinv
    
    if len(P) == len((P)[0]) and len(P) == 1:
        C[k] = numpy.dot( S[k], U[k] )
    else:
        C[k] = tripledot(S[k], U[k], P)
