...
for j in range(N): # range(1,N) --> [1,...,N-1]
    ...
    #X[j+1], delta[j], Y[j] = svd(B[j])  # B ---svd---> X delta Y
    #print svd(B[j])
    print "delta[",j,"] is ", delta[j]
    
    lam[j], Q[j], R[j] = svdLambda( delta[j] )   # delta ---plain decomposition---> Q lam R
    #Q[j], lam[j], R[j] = svd( delta[j] )   # delta ---plain decomposition---> Q lam R
    print "lam[",j,"] is ", lam[j]
    
    Mtilde[j] = numpy.dot( R[j] , Y[j] )    #numpy.tensordot( R[j] , Y[j], axes=2 )
