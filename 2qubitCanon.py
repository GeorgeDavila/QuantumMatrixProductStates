...
#Example MPS (all 1's)
M[1] = numpy.ndarray.tolist( numpy.ones( (1,2) ) )  # = M[N-1]
...
print R[0]
print Y[0]
Mtilde[0] = numpy.outer( R[0] , Y[0] )    #numpy.tensordot( R[0] , Y[0], axes=2 )
...
for j in range(1,N): # range(1,N) --> [1,...,N-1]
    ...
    print R[j]
    print Y[j]
    Mtilde[j] = numpy.outer( R[j] , Y[j] )    #numpy.tensordot( R[j] , Y[j], axes=2 )
