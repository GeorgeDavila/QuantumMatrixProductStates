...
oldcatdatavec = [None]*(x0) #x defined as len(datavec), so defines a list of same length as the datavec (for now filled with None's)
...
for i in range(1,Ngenes+1):               # range(NumSampleMin, (NumSampleMax + 1) ) only correct when we start at NumSampleMin = 1 
    oldcatdatavec[(i-1)::Ngenes] = list(numpy.loadtxt('PCV1_Sample_' + str(i) + '.txt'))


del oldcatdatavec[(cutgenelength*Ngenes) : x0+1 ] #Deletes from the (cutgenelength*Ngenes)-th part to the last term

x = len(oldcatdatavec)

catdatavec = numpy.ndarray.tolist(numpy.array(oldcatdatavec) + 1.0)
