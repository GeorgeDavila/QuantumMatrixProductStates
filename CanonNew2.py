...
#-----------Rightmost----------------------------
B[0] = numpy.concatenate((M[0],M[0]) , axis=1 )
print "B[",0,"] is ", B[0]
...
rmax = max( len(diagdelta), len(diagdelta[0]) ) #should be number of rows in rightmost case
Q[0] = numpy.eye( rmax,rmax )
    
rmin = min( len(diagdelta), len(diagdelta[0]) ) #should be number of columns in rightmost case
...
for j in range(1,N): # range(1,N) --> [1,...,N-1]
    B[j] = numpy.ndarray.tolist( numpy.dot( M[j] , Z[j] ) )#numpy.concatenate(( numpy.dot( M[j] , Z[j] ) , numpy.dot( M[j] , Z[j] ) ) , axis=1 ) #numpy.dot( M[j] , Z[j] )
    print "B[",j,"] is ", B[j]
    ...
    #For rightmost case just want to set lam[0]= delta[0]
    diagdelta = delta[j]
    lam[j] = numpy.diag(numpy.diag(diagdelta))
    
    if len(diagdelta) == max( len(diagdelta), len(diagdelta[j]) ):
        Q[j] = numpy.eye( len(diagdelta) , len(diagdelta[j]) )
    else:
        Q[j] = numpy.eye( len(diagdelta), len(diagdelta) )
    
    if len(diagdelta[j]) == max( len(diagdelta), len(diagdelta[j]) ):
        p[j] = numpy.eye( len(diagdelta), len(diagdelta[j]) )
    else:
        p[j] = numpy.eye( len(diagdelta[j]), len(diagdelta[j]) )
    #-----------------Secondary Diagonalization----------------------------------------------------------------------------------------------
