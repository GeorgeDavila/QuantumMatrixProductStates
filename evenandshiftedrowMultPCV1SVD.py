...
#In this program the size of the catenated datavectors are made to be even 
#Also use shifted row construction for W's 
...
'''if x0 % 2 == 1:  #odd
    print 'The length of this datavec was odd so we excise the last term of each gene from the datavector. The terms we excise are ', datavec[(genelength - 1):: genelength ]
    del datavec[(genelength - 1):: genelength ] #deletes the last element of each gene (Assuming genes are of same length, if they aren't a warning will be printed above)
    x = len(datavec)
else: #even
    x = x0
...
print 'The length of this datavec is now, len(datavec) = ', len(datavec) , ', a value which should be equal to 1759*Ngenes = ', 1759*Ngenes #Since PCV1 has 1759 base pairs 
...
if x0 % 2 == 1:  #odd #deletes the last element of each gene (Assuming genes are of same length, if they aren't a warning will be printed above)
    print 'The length of this datavec (and thus catenated datavec) was odd so we excise the last term of each gene from the datavector. The terms we excise are ', datavec[(genelength - 1):: genelength ], ' or in terms of the catenated datavec: ', catdatavec[-Ngenes: -1 ], ' and ', catdatavec[-1], ' (these should match the previous list)' 
    del catdatavec[-Ngenes : -1 ] 
...
print 'The Length of catdatavec is now ', x, '(which should be even)'
...
print "This sample has ", ( (2**b) - x ), " or ", (( (2**b) - 1759*Ngenes )/( (2**b) ) )*100, "percent filler terms"
...
for B in range(int(b)-1,0,-1):   #NOTE: Start at int(b)-1 since we did defined W0 previously, so this one should start by making W1
    ...
    SVDtxtfilename = 'Even' + 'C' + str(NumSampleMin) + str(NumSampleMax) + 'SingValsofSVDnum' + str( int(b-B)+1 ) + '.txt'
